...
The large animated home title also owns a hidden Rainbow Mode toggle. Twenty consecutive taps on the title fades in `Kikis, this is for you` and then enables an animated rainbow background; another twenty title taps disables it and fades in `rainbow mode deactivated`. This mode is intentionally separate from palette selection and has no visible control.
